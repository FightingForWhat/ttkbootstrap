...
class Tableview(ttk.Frame):
    ...
    def __init__(
        ...
        self._bottom_scrollbar = bottom_scrollbar
    ...
    def show_data(self):
        rowcount = len(self._tablerows)
        page_start = rowcount - 1
        row_data = self._tablerows[page_start]
        if self._stripecolor is not None and rowcount % 2 == 0:
            row_data.show(True)
        else:
            row_data.show(False)
        self._viewdata.append(row_data)
    ...
    def view_down(self):
        try:
            iid = self.tablerows_visible[-1].iid
    ...
    def _build_tableview_widget(self, coldata, rowdata, bootstyle):
        ...
        if self._bottom_scrollbar:
            self.h_bar = ttk.Scrollbar(
                master=self, command=self.view.xview, orient=HORIZONTAL
            )
            self.h_bar.pack(fill=X)
            self.view.configure(xscrollcommand=self.h_bar.set)

        if self._vertical_scrollbar:
            self.v_bar = ttk.Scrollbar(
                bootstyle="round", master=self, command=self.view.yview, orient=VERTICAL
            )
            self.v_bar.pack(side=RIGHT, fill=Y)
            self.view.configure(yscrollcommand=self.v_bar.set)
...
class TableCellRightClickMenu(tk.Menu):
    ...
    def __init__(self, master: Tableview):
        ...
        self.add_cascade(menu=filter_menu, label='⧨  过滤')
        self.add_separator()
        self.add_cascade(cnf=config['sortascending'])
        self.add_cascade(cnf=config['sortdescending'])
        self.add_separator()
        self.add_command(cnf=config["exportall"])
        self.add_command(cnf=config["exportselection"])
        self.add_separator()
...
class TableHeaderRightClickMenu(tk.Menu):
    ...
    def hide_column(self):
        ...
        eo = self.master._get_event_objects(self.event)
        eo.column.hide()
sociated with the cid is deleted
        regardless of whether it is in the visible data sets.
        Parameters:
            index (int):
                The numerical index of the column.
